...
module VersaLex
  ...
  class Event
    ...
    #--------------------------------------------------------------------------#
    # Renders the event as a json string.                                      #
    #--------------------------------------------------------------------------#
    def inspect_json
        msg = @attributes ? @attributes : {}
        msg['threadid'] = @threadid if @threadid
        msg['command'] = @command if @command
        msg['time'] = @time.strftime('%FT%T%:z') if @time
        msg['id'] = @id if @id
        msg.to_json
    end
...
if __FILE__ == $0
  ...
  parser = OptionParser.new do |opts|
    ...
    opts.on("-j", "--json", "print log output in json") do |j|
      options[:json] = j
    end
  ...
  laster   = Laster.new(eventlog, options[:last])    {|e| puts options[:json] ? e.inspect_json : e.inspect_colorfully} if options[:last]
  dumper   = Dumper.new(eventlog, options[:skip]||0) {|e| puts options[:json] ? e.inspect_json : e.inspect_colorfully}
  begin
    eventlog.run
  rescue Exception => e
